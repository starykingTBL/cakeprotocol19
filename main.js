...
const MUSIC_FALLBACK = 'https://cdn.pixabay.com/audio/2022/03/15/audio_8cb8b4a6b6.mp3';
...
const sfxFill       = $('#sfx-fill');
const btnSfxUp      = $('#btn-sfx-up');
const btnSfxDown    = $('#btn-sfx-down');
...
let musicVolume=0.7, muted=false;
...
btnVolUp.addEventListener('click',()=>{
  ...
  bgMusic.volume=musicVolume; muted=false; bgMusic.muted=false;
  updateVolUI(); SFX.click();
});
btnVolDown.addEventListener('click',()=>{
  musicVolume=Math.max(0,musicVolume-0.1);
  bgMusic.volume=musicVolume; updateVolUI(); SFX.click();
});
btnSfxUp.addEventListener('click',()=>{
  sfxVolume=Math.min(1,sfxVolume+0.1); updateVolUI(); SFX.click();
});
btnSfxDown.addEventListener('click',()=>{
  sfxVolume=Math.max(0,sfxVolume-0.1); updateVolUI(); SFX.click();
});
...
const Ambient=(()=>{
  ...
  function draw(ts){
    ...
    for(const p of parts){
      ...
      p.o=Math.max(0.05,Math.min(0.45,p.o+p.do));
      if(p.o<=0.05||p.o>=0.45)p.do*=-1;
      p.a+=0.004;
...
async function phaseCake(){
  ...
  for(let i=0;i<GREETING_LINES.length;i++){
    ...
    await wait(30); line.classList.add('show');
    SFX.reveal();
  }
  await wait(800);
...
async function phaseEnvelope(){
  SFX.whoosh(); hideStage(stageCake);
  await wait(400);
...
async function phaseArcade(){
  SFX.arcade(); hideStage(stageLetter);
  await wait(500);
...
function drawWaveform(){
  ...
  const W=canvas.offsetWidth||300, H=canvas.offsetHeight||58;
  canvas.width=W; canvas.height=H;
  const bars=64, gap=2, bw=(W-(bars-1)*gap)/bars;
  ctx.clearRect(0,0,W,H);
  for(let i=0;i<bars;i++){
    const h=(Math.random()*0.65+0.15)*H;
    const x=i*(bw+gap), y=(H-h)/2;
    ctx.fillStyle='rgba(139,26,42,0.55)';
    ctx.beginPath();
...
vnPlayBtn.addEventListener('click',()=>{
  if(vnAudio.paused){ vnAudio.play().catch(()=>{}); vnIconPlay.style.display='none'; vnIconPause.style.display='block'; }
  else { vnAudio.pause(); vnIconPlay.style.display='block'; vnIconPause.style.display='none'; }
  SFX.click();
});
...
function openGame(key){
  ...
  if(window.GameRegistry[key]){
    ...
    activeGame.mount(gameMount);
  } else {
    gameMount.innerHTML=`<div class="game-placeholder"><span class="game-placeholder-label">loading</span><p class="game-placeholder-title">${meta.title}</p></div>`;
  }
}
function closeGame(){
  SFX.click();
