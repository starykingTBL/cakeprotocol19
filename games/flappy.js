...
window.GameRegistry['flappy']={
  mount(container){
    ...
    const GRAVITY=0.5, JUMP=-9, PIPE_W=52, GAP=135, PIPE_SPD=2.6;
    let bird,pipes,score,best=0,raf,running=false,stars;

    function mkStars(){ return Array.from({length:35},()=>({x:Math.random()*W,y:Math.random()*H,r:Math.random()*1.2+0.3,o:Math.random()*0.5+0.2})); }

    function startGame(){
      bird={x:80,y:H/2,vy:0,r:14,angle:0};
    ...
    function update(){
      bird.vy+=GRAVITY;
      bird.y+=bird.vy;
      bird.angle=Math.max(-0.5,Math.min(1.2,bird.vy*0.06));

      if(bird.y-bird.r<0){ bird.y=bird.r; bird.vy=0; }
      if(bird.y+bird.r>H){ die(); return; }

      if(!pipes.length||pipes[pipes.length-1].x<W-180){
    ...
    function die(){
      running=false;
      if(sfx.die) sfx.die();
      msg.innerHTML=`<p class="game-msg-title">Game Over</p><p class="game-msg-sub">Score: ${score}</p><button class="game-msg-btn" id="fl-start">Try Again</button>`;
      msg.style.display='flex';
      msg.querySelector('#fl-start').addEventListener('click',startGame);
    }

    function drawPipe(x,h,flip){
      const g=ctx.createLinearGradient(x,0,x+PIPE_W,0);
      g.addColorStop(0,'#2a0d14'); g.addColorStop(0.5,'#6b1a2a'); g.addColorStop(1,'#2a0d14');
      ctx.fillStyle=g;
      ctx.beginPath();
      if(ctx.roundRect) ctx.roundRect(x,flip?0:H-h,PIPE_W,h,flip?[0,0,6,6]:[6,6,0,0]);
      else ctx.rect(x,flip?0:H-h,PIPE_W,h);
      ctx.fill();
      ctx.fillStyle='#8b1a2a';
      const capH=14;
      ctx.beginPath();
      if(ctx.roundRect) ctx.roundRect(x-4,flip?h-capH:H-h,PIPE_W+8,capH,4);
      else ctx.rect(x-4,flip?h-capH:H-h,PIPE_W+8,capH);
      ctx.fill();
    }

    function draw(){
      const sky=ctx.createLinearGradient(0,0,0,H);
      sky.addColorStop(0,'#050208'); sky.addColorStop(1,'#150810');
      ctx.fillStyle=sky; ctx.fillRect(0,0,W,H);
      stars.forEach(s=>{
        ctx.globalAlpha=s.o*(0.7+Math.sin(Date.now()*0.001+s.x)*0.3);
        ctx.fillStyle='#fff';
      ...
      pipes.forEach(p=>{
        ...
        drawPipe(p.x,H-p.top-GAP,false);
      });
      ctx.save();
      ctx.translate(bird.x,bird.y);
      ctx.rotate(bird.angle);
      const bg=ctx.createRadialGradient(-2,-2,0,0,0,bird.r);
      bg.addColorStop(0,'#ffd166'); bg.addColorStop(1,'#c9a96e');
      ctx.fillStyle=bg;
      ...
      ctx.beginPath(); ctx.moveTo(10,2); ctx.lineTo(16,0); ctx.lineTo(10,4); ctx.fill();
      ctx.restore();
