...
window.GameRegistry['runner']={
  mount(container){
    ...
    container.innerHTML=`
      <div class="game-ui">
        ...
        <div class="game-msg" id="rn-msg">
          ...
          <p class="game-msg-sub">Tap or press Space to jump</p>
    ...
    function mkStars(){ return Array.from({length:40},()=>({x:Math.random()*W,y:Math.random()*(GND-20),s:Math.random()*1.5+0.3,sp:Math.random()*0.3+0.1})); }

    function startGame(){
      player={x:60,y:GND,w:22,h:28,vy:0,onGround:true,jumps:0};
      obstacles=[]; particles=[]; stars=mkStars();
      score=0; speed=3.2; frame=0; running=true;
    ...
    function jump(){
      ...
      if(player.jumps<2){
        player.vy=-11; player.onGround=false; player.jumps++;
    ...
    function spawnParticle(x,y){
      for(let i=0;i<4;i++) particles.push({x,y,vx:(Math.random()-0.5)*3,vy:-Math.random()*3-1,life:1});
    }
    ...
    function die(){
      running=false;
      if(sfx.die) sfx.die();
      msg.innerHTML=`<p class="game-msg-title">Game Over</p><p class="game-msg-sub">Score: ${score}</p><button class="game-msg-btn" id="rn-start">Try Again</button>`;
      msg.style.display='flex';
      msg.querySelector('#rn-start').addEventListener('click',startGame);
    }

    function draw(){
      ctx.fillStyle='#0a0407'; ctx.fillRect(0,0,W,H);
      stars.forEach(s=>{
        ctx.fillStyle=`rgba(255,200,200,${s.s*0.4})`;
        ctx.beginPath(); ctx.arc(s.x,s.y,s.s,0,Math.PI*2); ctx.fill();
      });
      const g=ctx.createLinearGradient(0,GND+28,0,H);
      g.addColorStop(0,'#1a0810'); g.addColorStop(1,'#0a0407');
      ctx.fillStyle=g; ctx.fillRect(0,GND+28,W,H-GND-28);
      ctx.fillStyle='#2a0d14'; ctx.fillRect(0,GND+28,W,3);
      obstacles.forEach(o=>{
        ctx.fillStyle=o.type==='tall'?'#4a1020':'#8b1a2a';
        ctx.beginPath();
        if(ctx.roundRect) ctx.roundRect(o.x,o.y,o.w,o.h,3);
        else ctx.rect(o.x,o.y,o.w,o.h);
        ctx.fill();
      });
      ctx.fillStyle='#c9a96e';
      ctx.beginPath();
      if(ctx.roundRect) ctx.roundRect(player.x,player.y,player.w,player.h,5);
      else ctx.rect(player.x,player.y,player.w,player.h);
      ctx.fill();
