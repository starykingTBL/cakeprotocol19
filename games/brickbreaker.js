...
window.GameRegistry['brickbreaker']={
  mount(container){
    container.innerHTML=`<div class="game-ui"><div class="game-score-bar"><span class="game-score-label">Score</span><span class="game-score-val" id="bb-sc">0</span><span class="game-score-label">Level</span><span class="game-score-val" id="bb-lv">1</span></div><div class="game-canvas-wrap" id="bb-wrap"><canvas id="bb-c" class="game-canvas"></canvas><div class="game-msg" id="bb-msg"><p class="game-msg-title">Brick Breaker</p><p class="game-msg-sub">Move paddle to break all bricks</p><button class="game-msg-btn" id="bb-s">Play</button></div></div></div>`;
    const canvas=container.querySelector('#bb-c'),wrap=container.querySelector('#bb-wrap'),ctx=canvas.getContext('2d'),scEl=container.querySelector('#bb-sc'),lvEl=container.querySelector('#bb-lv'),msg=container.querySelector('#bb-msg'),sfx=window.SFX||{};
    const resize=()=>{canvas.width=wrap.clientWidth||320;canvas.height=wrap.clientHeight||400;};
    resize();window.addEventListener('resize',resize);
    let pad,ball,bricks,sc=0,lv=1,raf,run=false;
    const PW=()=>Math.min(90,canvas.width*.28);const PH=14;
    const mkBricks=l=>{const cols=6,bw=(canvas.width-20)/cols-4,bh=18,rows=3+Math.min(l-1,4),arr=[];for(let r=0;r<rows;r++)for(let c=0;c<cols;c++){const t=l>3&&Math.random()<.15?2:l>1&&Math.random()<.2?1:0;const hp=[1,2,0][t];arr.push({x:10+c*(bw+4),y:45+r*(bh+5),w:bw,h:bh,hp,mhp:hp,t});}return arr;};
    const startLv=()=>{const spd=3.5+lv*.5,pw=PW();pad={x:canvas.width/2-pw/2,y:canvas.height-32,w:pw,h:PH};ball={x:canvas.width/2,y:canvas.height-60,vx:spd*(Math.random()<.5?.7:-.7),vy:-spd,r:8};bricks=mkBricks(lv);run=true;msg.style.display='none';if(raf)cancelAnimationFrame(raf);loop();};
    const startGame=()=>{sc=0;lv=1;lvEl.textContent=1;scEl.textContent=0;startLv();};
    ...
    const update=()=>{
      ...
      if(ball.y+ball.r>=pad.y&&ball.y+ball.r<=pad.y+pad.h+ball.r*2&&ball.x>=pad.x-ball.r&&ball.x<=pad.x+pad.w+ball.r){
        ball.vy=-Math.abs(ball.vy);ball.vx=((ball.x-(pad.x+pad.w/2))/(pad.w/2))*5;ball.y=pad.y-ball.r;
    ...
    const nextLv=()=>{run=false;cancelAnimationFrame(raf);lv++;if(sfx.arcade)sfx.arcade();lvEl.textContent=lv;msg.innerHTML=`<p class="game-msg-title">Level ${lv-1} Clear!</p><p class="game-msg-sub">Score: ${sc}</p><button class="game-msg-btn" id="bb-s">Next</button>`;msg.style.display='flex';msg.querySelector('#bb-s').addEventListener('click',startLv);};
    ...
    canvas.addEventListener('mousemove',onMove);canvas.addEventListener('touchmove',e=>{e.preventDefault();onMove(e);},{passive:false});
    container.querySelector('#bb-s').addEventListener('click',startGame);draw();
